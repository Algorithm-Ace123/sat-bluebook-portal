...
export async function POST(req: Request) {
    ...
    const isFRQ = question.kind === "frq_math";

    const systemPrompt = isFRQ
        ? "You are Pramana Bot, an expert AI SAT tutor. You must ONLY return a raw JSON object. NO markdown fences outside the JSON. The JSON must exactly follow this schema: { \"correct_rationale\": \"string explaining mathematical steps for why correct answer is correct\", \"incorrect_rationales\": [], \"tips\": \"string with helpful tip\" }. Because this is a free-response question, do not attempt to guess incorrect options."
